...
resource "coder_agent" "main" {
  ...
  startup_script = <<-EOT
    ...
    # Write Claude Code credentials file from the full credential JSON env var.
    # Claude Code reads ~/.claude/.credentials.json, not env vars.
    if [ -n "$CLAUDE_CODE_CREDENTIALS" ]; then
      mkdir -p /home/vscode/.claude
      echo "$CLAUDE_CODE_CREDENTIALS" > /home/vscode/.claude/.credentials.json
      chmod 600 /home/vscode/.claude/.credentials.json
    fi

    # Propagate secret-injected env vars into login shells (Coder web terminal
    # doesn't inherit container env when envbuilder rebuilds the image).
    # Reads every key from the mounted coder-workspace-env secret and writes
    # to /etc/profile.d/ so any new shell gets them automatically.
    ENV_FILE="/etc/profile.d/coder-workspace-env.sh"
    {
      echo '#!/bin/bash'
      echo '# Auto-generated by Coder startup script from coder-workspace-env secret'
      for var in $(env | cut -d= -f1); do
        case "$var" in
          HOME|USER|SHELL|PATH|PWD|OLDPWD|SHLVL|TERM|HOSTNAME|_|LOGNAME) continue ;;
          SSH_*|CODER_AGENT_*|KUBERNETES_*|CODER_CNPG_*|CODER_PORT*|CODER_SERVICE_*) continue ;;
          CODER|CODER_WORKSPACE_*|ENVBUILDER*|DOCKER_BUILDKIT|DEVCONTAINER*) continue ;;
          GIT_*|NVM_*|GOPATH|GOROOT|PYTHON_PATH|PIPX_*|CS_*|TS_DEBUG_*) continue ;;
    ...
    # Also source from .bashrc for non-login shells (VS Code integrated terminal)
    grep -q 'coder-workspace-env.sh' /home/vscode/.bashrc 2>/dev/null || \
      echo '[ -f /etc/profile.d/coder-workspace-env.sh ] && . /etc/profile.d/coder-workspace-env.sh' >> /home/vscode/.bashrc
  EOT
