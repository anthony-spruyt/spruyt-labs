...
- **Community `podman` devcontainer feature** — third-party, may misbehave under envbuilder's build-time feature execution; inline install in `setup-devcontainer.sh` matches existing repo idiom (Claude CLI, safe-chain) and pins versions explicitly.
...
- `apk add --no-cache openssh-keygen github-cli ca-certificates` at build time
