...
Add a Claude Code execution layer to the existing n8n deployment. n8n workflows
spawn ephemeral pods in a dedicated `claude-agents` namespace that run `claude -p`
with native Claude Code configuration (setup token auth, MCP servers, settings).
This is the foundation for phase 2's operational workflows (alertmanager triage,
GitHub issue triage, etc.).
...
│  1. kubectl get secret → export CLAUDE_CODE_OAUTH_TOKEN     │
...
└─────────────────────────────────────────────────────────────┘
              ...
              ▼
┌─ External ──────────────────────────────────────────────────┐
│  api.anthropic.com (Claude API — setup token auth)          │
...
Contains a static setup token generated by `claude setup-token`. This token is
valid for 1 year and does not require refresh — avoiding the race conditions
inherent in OAuth refresh token rotation across multiple ephemeral pods.

**Why not `.credentials.json`?** Standard OAuth tokens from `claude login` use
rotating single-use refresh tokens. When pod A refreshes, pod B's copy is
permanently invalidated. Additionally, `claude -p` (headless mode) has a known
bug where it does not auto-refresh tokens
([#37402](https://github.com/anthropics/claude-code/issues/37402)). The setup
token avoids both problems.

**Generation:** Run `claude setup-token` interactively once, copy the token,
encrypt into the SOPS secret. Rotate annually.
...
type: Opaque
stringData:
  oauth-token: "cct_..."  # from claude setup-token (1-year lifetime)
```
...
4. Claude authenticates via setup token (`CLAUDE_CODE_OAUTH_TOKEN`)
...
- The entrypoint script reads a secret via `kubectl get secret -o jsonpath` to
  ...
  `kubectl get secret -o jsonpath` applies to Claude's behavior, not to
...
| Concern             | Mitigation                                                                    |
| ------------------- | ----------------------------------------------------------------------------- |
| Credential exposure | Setup token in SOPS-encrypted secret, exported as env var at bootstrap only   |
...
5. **~~OAuth token refresh lifecycle~~** — **Resolved.** Research found that
   standard OAuth tokens use rotating single-use refresh tokens (race condition
   across pods) and `claude -p` doesn't auto-refresh (known bug #37402). Solution:
   use `claude setup-token` which generates a 1-year static token stored as
   `CLAUDE_CODE_OAUTH_TOKEN` env var. No refresh needed. Annual rotation.
