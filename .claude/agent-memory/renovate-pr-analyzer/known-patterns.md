...
| coder/coder | Two-channel release model: one `vX.Y.Z` tag is marked `Latest` (stable) while a newer `vX.(Y+1).0` is "mainline" and explicitly recommends staging environments. Release notes include a `[!NOTE]` banner: "This is a mainline Coder release. We advise enterprise customers without a staging environment to install our latest stable release." Minor bumps from stable → mainline (e.g., 2.30.7 → 2.32.0 skipping 2.31.x) warrant RISKY on release-maturity grounds even when specific breaking changes have NO_IMPACT. Prefer pinning to the `Latest`-tagged stable line or wait for 2.x.z patches. | 2 | 2026-04-19 | 2026-04-15 |
