...
| cloudnative-pg/postgresql image | Renovate PR title can be misleading: `update to 18` may mean "digest bump within PG18" (both old and new digests share tag `:18`), NOT a PG17→PG18 major bump. Always diff the full tag+digest — if the tag string is identical, it is a digest-only refresh. | 1 | 2026-04-15 | 2026-04-15 |
...
| coder/coder | Two-channel release model: one `vX.Y.Z` tag is marked `Latest` (stable) while a newer `vX.(Y+1).0` is "mainline" and explicitly recommends staging environments. Release notes include a `[!NOTE]` banner: "This is a mainline Coder release. We advise enterprise customers without a staging environment to install our latest stable release." Minor bumps from stable -> mainline warrant RISKY on release-maturity grounds even when specific breaking changes have NO_IMPACT. As of 2026-04-19, v2.31.9 is `Latest` (stable), v2.32.0 is mainline. Individual v2.31.x release notes still carry the mainline banner even though the line has been promoted — check `gh release list` to determine actual stable status. | 3 | 2026-04-19 | 2026-04-15 |
...
| `ghcr.io/cloudnative-pg/postgresql` (CNPG postgres image) | `cloudnative-pg/postgres-containers` (Debian/<major>/bookworm path; daily automated rebuilds; no semantic releases — digest-only updates are common). Runtime/operator bugs: `cloudnative-pg/cloudnative-pg`. | 1 | 2026-04-15 | 2026-04-15 |
...
| CNPG postgres image digest bump while `plugins: barman-cloud.cloudnative-pg.io` is enabled | Open upstream bug cloudnative-pg/cloudnative-pg#9770: rolling image updates stall on the last replica when plugin-barman-cloud is installed, leaving the cluster degraded (N-1/N ready) indefinitely. All our CNPG clusters (authentik, coder, firefly-iii, n8n) use this plugin. HIGH_IMPACT until upstream fix ships. Mitigation: manually delete the stuck replica pod after update, or wait for fix. | 1 | 2026-04-15 | 2026-04-15 |
