...
| MegaLinter golangci-lint uses project mode, not file mode | Custom CLI_EXECUTABLE wrapper receives `run [args]` as argv, not individual .go file paths | golangci-lint descriptor defaults to `cli_lint_mode: project`. Wrapper scripts expecting per-file invocation must set `GO_GOLANGCI_LINT_CLI_LINT_MODE: file` or handle `run` subcommand. False-pass risk: wrapper exits 0 without linting | 1 | 2026-03-25 | 2026-03-25 |
| MegaLinter container Go version lags behind go.mod requirements | Container ships Go 1.25.7 but modules require 1.26.1, causing `go mod download` failures | Add `GOTOOLCHAIN=auto` to env exports so Go toolchain auto-downloads. Masked by `\|\| true` in pre-commands | 1 | 2026-03-25 | 2026-03-25 |
