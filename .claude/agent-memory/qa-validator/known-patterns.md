...
| MegaLinter cannot run inside devcontainer due to nested rootless podman (newuidmap) | `task dev-env:lint` fails with `cannot setup namespace using newuidmap: exit status 1` when run from inside a container host that lacks shared `/` mount / subuid mapping for the nested podman | Fall back to running individual pre-commit hooks via `pre-commit run --files <paths>` on the specific changed files; note MegaLinter skip explicitly in report. Backgrounded task exit 0 from `task` wrapper can mask the real podman exit 125 -- always read the output, don't trust task exit status alone | 2 | 2026-04-16 | 2026-04-16 |
