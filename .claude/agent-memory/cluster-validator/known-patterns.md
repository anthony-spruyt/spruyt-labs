...
| Large-scale VPA rollout (50 files, 86 objects) reconciles within ~5 min across all namespaces; VPA objects with `updateMode: Off` show recommendations within minutes of creation; newly created VPAs may show empty status initially | VPA workload rollout, kustomization type | 1 | 2026-03-18 | 2026-03-18 |
...
| MCP detection tools (certs_detect_tool, cilium_endpoints_list_tool, cilium_status_tool) fail with Forbidden because they internally call `list CRDs` via apiextensions.k8s.io; direct list_custom_resources for specific API groups works correctly with per-group RBAC | kubectl-mcp-server RBAC, MCP tool detection vs direct query | 1 | 2026-03-18 | 2026-03-18 |
