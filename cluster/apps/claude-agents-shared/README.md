...
| `inject-shared-config`    | all 5       | gh CLI config, gitconfig, settings profiles, managed-settings, claude-home emptyDir, bootstrap-script ConfigMap, Context7 key, OTEL env vars |
...
| `inject-repo-clone-write` | write tiers | SSH clone init container + pre-commit install + plugin-bootstrap init container                                                              |
| `inject-repo-clone-read`  | read + sre  | HTTPS clone init container (token-authenticated) + plugin-bootstrap init container                                                           |
