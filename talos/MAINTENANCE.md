...
### Node Maintenance Decision Tree

```yaml
start: "maintenance_needed"
nodes:
  maintenance_needed:
    question: "What type of node maintenance is needed?"
    options:
      add_node: "Add new node"
      remove_node: "Remove existing node"
      upgrade_os: "Upgrade Talos OS"
      upgrade_k8s: "Upgrade Kubernetes"
  add_node:
    action: "Follow node addition procedure with hardware prep and config generation"
    next: "validate_maintenance"
  remove_node:
    action: "Drain workloads, migrate data, remove from cluster"
    next: "validate_maintenance"
  upgrade_os:
    action: "Upgrade control plane first, then workers, validate each step"
    next: "validate_maintenance"
  upgrade_k8s:
    action: "Run dry-run, execute upgrade, monitor components"
    next: "validate_maintenance"
  validate_maintenance:
    ...
    no: "rollback_needed"
  rollback_needed:
    action: "Execute appropriate rollback procedure"
    next: "validate_maintenance"
