...
inject_local_storage_encryption() {
  local file="$1"
  cat <<'EOF' >> "$file"
encryption:
  provider: luks2
...
#echo "Injecting local-storage encryption..."
#for config in /workspaces/spruyt-labs/talos/clusterconfig/*-e2-*.yaml; do
#  inject_local_storage_encryption "$config"
#done
