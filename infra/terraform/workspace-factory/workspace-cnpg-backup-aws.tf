data "aws_iam_policy_document" "cnpg_backup_tfc_aws_iam_policy" {
  version = "2012-10-17"
  statement {
    sid    = "CnpgBackupTerraformAccess"
    effect = "Allow"
    actions = [
      "*"
    ]
...
module "cnpg_backup" {
  ...
  tfc_trigger_pattern                     = var.cnpg_backup_aws_tfc_trigger_pattern
  ...
  tfc_working_directory                   = var.cnpg_backup_aws_tfc_working_directory
  tfc_workspace_name                      = var.cnpg_backup_aws_tfc_workspace_name
